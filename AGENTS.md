...
- **Mermaid diagrams:** add `mermaid: true` to frontmatter to render ```` ```mermaid ```` fences; the script is injected by `layouts/partials/hooks/body-end.html`. Without the flag the fence renders as plain code. If node labels come out clipped mid-word, add `mermaid_html_labels: true` too: mermaid's default `securityLevel: 'strict'` implies `htmlLabels: false`, and it then under-measures labels containing accents or symbols. The flag switches that page to `securityLevel: 'antiscript'` + `htmlLabels: true` (browser-measured text, and `<br/>` works). It is opt-in per post so that already-published diagrams do not silently re-flow.
- **Math formulas:** write LaTeX directly — `\( ... \)` inline, `\[ ... \]` or `$$ ... $$` for display. Rendered to native MathML **at build time** by `layouts/_markup/render-passthrough.html` using `transform.ToMath`; no JavaScript, no CDN, no CSS, no frontmatter flag. Depends on `[markup.goldmark.extensions.passthrough]` being enabled in `hugo.toml` — without it the markdown parser eats the `_` and `\` before the math renderer runs.
...
**Cross-references between drafts** use `[[ID]]` notation (e.g., `[[A1-03]]`, `[[tr-07]]`, `[[K-12]]`) during drafting; they resolve to real `[texto](/posts/slug/)` URLs at publication time. Each post has a stable ID `{series}-{NN}`.

**Internal link URLs never contain the date.** The bundle is `content/es/posts/YYYY-MM-DD-slug/`, but Hugo strips the date prefix when building the permalink, so the published URL is `/posts/slug/`. The dated form 404s — no alias is generated for it.
...
- **Custom layouts:**
  ...
  - `layouts/partials/hooks/body-end.html` — hosts two independent opt-in/opt-out blocks: the mermaid loader (`mermaid: true`, with the `mermaid_html_labels` variant described in Content conventions) and the read-aloud audio player (opt-**out** via `audio_player: false`)
  - `layouts/_markup/render-passthrough.html` — renders LaTeX passthrough blocks to native MathML at build time via `transform.ToMath`. Requires `[markup.goldmark.extensions.passthrough]` in `hugo.toml`. Emits `output: "mathml"`, so no math JS, no KaTeX CSS and no webfonts reach the browser. Path note: Hugo 0.146+ expects `layouts/_markup/`, not the older `layouts/_default/_markup/`
  - `layouts/partials/site-header.html` — overrides the theme's site header. Two customizations: (1) in the *no-featured-image* branch (homepage and any post without `featured_image`), trims the tall black banner on desktop — outer `pb6-l` → `pb2-l`, title `f-subheadline-l` → `f1-l`, subtitle `f3-l` → `f4-l` with margins `mt3 mb4` → `mt2 mb2` (mobile layout matches the theme default); (2) when the `<h1>` is showing the site title ("katra"), wraps it in an anchor + native `title=""` tooltip pointing to the explainer post in the page's language — ES → `/posts/por-que-katra/` ("¿Por qué katra?"), EN → `/en/posts/why-katra/` ("Why katra?"). Uses `relLangURL` to get the right language prefix; extend the `cond` block if a new language is added. Pages with their own `.Title` render unchanged.
- **Custom CSS:**
